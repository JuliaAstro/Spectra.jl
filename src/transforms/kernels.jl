...
Kernels serve as the backbone of broadening and convolution operations. All kernels have a common interface:

    evaluate(::Kernel; maxwidth=8, length=100)

which provides the evaluated PDF of the kernel. Note that the user should not have to provide the evaluated points, so each kernel should have a way of automatically choosing its range. For instance, a Gaussian kernel may choose to automatically evaluate 1000 points from -8σ to 8σ.
"""
